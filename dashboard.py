...
def plot_yearly(df, year):
    ...
    fig.suptitle(
        f"Birmingham - full year overview {year}",
        fontsize=14
    )
    for ax, (col, ylabel, colour, _) in zip(axes, PANELS):
        ...
        ax.axhline(
            mean_val, color=colour, linewidth=1,
            linestyle="--", alpha=0.7,
...
def plot_monthly(df, year):
    ...
    fig.suptitle(
        f"Birmingham - monthly summary {year}",
        fontsize=14
    )
    axes = axes.flatten()
    mp = range(1, 13)
    ...
    ax = axes[2]
    ax.bar(
        mp, monthly["windspeed"]["max"],
        color="lightgreen", alpha=0.5,
        width=0.6, label="Max"
    )
    ax.bar(
        mp, monthly["windspeed"]["mean"],
    ...
    ax = axes[3]
    ax.plot(
        mp, monthly["humidity"]["mean"],
        color="mediumpurple", linewidth=2.5,
        marker="o", markersize=5
    )
    ax.fill_between(
        mp, 60, monthly["humidity"]["mean"],
...
def plot_forecast(df_recent, df_forecast, today):
    ...
    for ax, (col, ylabel, c_hist, c_fcast) in zip(axes, PANELS):
        ...
        ax.plot(
            df_forecast.index, df_forecast[col],
...
def build_map(latest):
    ...
    popup_html = (
        ...
        "</td></tr>"
        "<tr><td><b>Rainfall</b></td><td>"
        + f"{latest['rainfall']:.1f} mm" +
        "</td></tr>"
        "<tr><td><b>Wind speed</b></td><td>"
...
def main():
    ...
    st.sidebar.title("Birmingham Weather")
    st.sidebar.markdown("---")
    ...
    if selected_tab == "Current conditions":
        ...
        st.caption(
            "Last updated: "
            + latest.name.strftime('%d %B %Y at %H:%M')
        )
        ...
        for year in YEARS:
            ...
            rows.append({
                "Year":               year,
                "Hot days over 25C":  int(
                    (dmax["temperature"] > 25).sum()
                ),
                "Frost days below 0C": int(
                    (dmin["temperature"] < 0).sum()
                ),
                "Heavy rain over 10mm": int(
                    (dsum > 10).sum()
                ),
                "High wind over 50kmh": int(
                    (dmax["windspeed"] > 50).sum()
                ),
            })
        st.dataframe(
